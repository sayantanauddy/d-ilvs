...
from panda_vision_imitation.data import from_tangent_plane, to_tangent_plane
...
# 4. python ros_scripts/bbox_tracker.py --tracked_obj_class mouse --prob_thresh 0.4 # <<< CHANGE OBJECT AND THRESHOLD AS NEEDED! (for cup 1 dataset, thresh=0.6, hist_len=50, without test_goal_center)
# 5. python ros_scripts/bbox_image_pub.py --test_center_goal
# 6. rosrun image_view image_view image:=/img_bbox_tracker
# 7. MOUSE: python ros_scripts/robot_NODE_YOLO.py --log_dir results_mouse_20231219 --description NODE_YOLO --gain 0.5 --max_steps 900 --rob_init --train_steps 500 --deadzone 0 --train_path data_node_mouse/train_data.npy --node_path logs/node_mouse/231212_150130_seed100/models/node.pt --desc pos0
# CUP: python ros_scripts/robot_NODE_YOLO.py --log_dir results_cup_solo_20240118 --description NODE_YOLO --gain 0.5 --max_steps 700 --rob_init --train_steps 500 --deadzone 0 --train_path data_node_cup_1/train_data.npy --node_path logs/node_cup_1/240117_182213_seed100/models/node.pt --desc pos1 --open_gripper
#
...
# 3. python ros_scripts/robot_NODE_YOLO.py --log_dir logs_ros --description NODE_YOLO --gain 0.5 --max_steps 900 --rob_init --train_steps 500 --deadzone 0 --train_path data_node_mouse/train_data.npy --node_path logs/node_mouse/231212_150130_seed100/models/node.pt --desc pos4

def parse():

    parser = argparse.ArgumentParser(description='Control panda using a YOLO controller')
    parser.add_argument('--description', type=str, required=True, help='Description of run')
    ...
    parser.add_argument('--record', action='store_true', help='Store the trajectory as an .npy file')    
...
class RobotNODEYOLO():
    ...
    def __init__(self):
        ...
        # Initialize ROS node
        rospy.init_node('NODE_YOLO_controller')
    ...
    def main(self):
        ...
        NUM_TRAIN_STEPS = self.args.train_steps
        NODE_PATH = self.args.node_path
        ROTVEC_SCALE = 100.0

        # Image size - must match size of images published by camera
        self.IMAGE_WIDTH = self.args.img_width
        self.IMAGE_HEIGHT = self.args.img_height

        # Dist between EE and top of cup measured manually at goal pose
        ...
        if self.args.rob_init:
            ...
            self.move_robot(
                pos_xyz=init_pos_xyz_cm/100.0, # convert position to m
        ...
        # Create t and delta_t for manual integration of NODE
        t = np.linspace(0.0, 1.0, NUM_TRAIN_STEPS)
        delta_t = np.ediff1d(t)[0]
        t = torch.from_numpy(t).float().to(device)

        # Create y_start
        pos_cm_xyz = init_pos_xyz_cm
        ...
        myprint(f'bbox_100_xyxy={bbox_100_xyxy}')
        myprint(f'pos_cm_xyz={pos_cm_xyz}')
        myprint(f'rot_vec_scaled_xyz={rot_vec_scaled_xyz}')

        # Initialize input for NODE
        y_start = np.concatenate([bbox_100_xyxy, pos_cm_xyz, rot_vec_scaled_xyz])
        y_start = torch.from_numpy(y_start).float().to(device)
        myprint(f'y_start={y_start}')
        ...
        # Init 
        # curr_quat_wxyz_first is not needed?
        ...
        # Load curr bboxes from file
        from panda_vision_imitation.logging import read_dict
        raw_yolo_file = '/home/sayantan/datasets/panda_vision_traj_dataset/demo_20230221_165021_select/yolo_results.json'

        # Read json and convert to list of lists
        raw_yolo_list = read_dict(raw_yolo_file)
        simple_yolo_list = [[d['xmin'], d['ymin'], d['xmax'], d['ymax']] for d in raw_yolo_list]
        filtered_yolo_list = list()
        # remove invalid values (for frames with missing object)
        for curr_bbox in simple_yolo_list:
            remove=False
            for e in curr_bbox:
                if e < 0:
                    remove = True
            ...
            if not remove:
                filtered_yolo_list.append(curr_bbox)
        filtered_yolo_np = np.array(filtered_yolo_list)

        # Storing Lv for debugging
        Lv_store = list()

        # Control loop
        # for i in range(self.args.max_steps):
        for i in range(filtered_yolo_np.shape[0]):
            ...
            #curr_bbox_xyxy4 = np.array([self.bbox.xmin, self.bbox.ymin, self.bbox.xmax, self.bbox.ymax])
            #assert_square(curr_bbox_xyxy4)

            curr_bbox_xyxy4 = filtered_yolo_np[i]
            ...
            # DEBUG Shape of Ls=(8, 6), Make last 3 columns 0
            #Ls[:,3:6] = 0.0
            ...
            # Compute a plot of the all 6 elements of Lv (one with Ls is 0d, and one where Ls is not 0d)
            Lv_store.append(Lv)
            ...
            myprint2(f'sCurr={sCurr}')
            myprint2(f'sCurr={sGoal}')
            myprint2(f'error={error}')

            myprint2(f'normE={normE}')

            myprint2(f'Shape of Ls={Ls.shape}')
            myprint2(f'Ls=\n{Ls}')

            myprint2(f'Shape of Ps={Ps.shape}')
            myprint2(f'Ps=\n{Ps}')
            
            myprint2(f'Shape of Lv={Lv.shape}')
            myprint2(f'Lv=\n{Lv}')
            
            myprint2(f'Shape of LvPinv={LvPinv.shape}')
            myprint2(f'LvPinv=\n{LvPinv}')

            myprint2(f'Shape of Pv={Pv.shape}')
            myprint2(f'Pv=\n{Pv}')

            # Save Lv to disk
            Lv_store_np = np.array(Lv_store)
            myprint2(os.path.join(self.args.log_dir,'Lv_store.npy'))
            np.save(os.path.join(self.args.log_dir,'Lv_store.npy'), Lv_store_np)
            
            if i==filtered_yolo_np.shape[0]-1:
                exit(0)
            ...
            # Task commanded by the bounding box error
            ...
            vBase_YOLO = b_V_e @ vEnd
            myprint(f'vBase={vBase_YOLO}')

            # One step through the target network of NODE
            d_y = node.target_network(y_start)
            myprint(f'd_y shape = {d_y.shape}')
            myprint(f'd_y = {d_y}')

            # In d_y, convert positions from m to cm, 
            # remove ROTVEC_SCALE from rotation vectors
            # Remove bbox part
            d_y_descaled = d_y[4:].detach().cpu().numpy() # Removing bbox
            d_y_descaled[0:3] /= 100.0 # Converting pos vels to m/s
            d_y_descaled[3:6] /= ROTVEC_SCALE # Removing ROTVEC_SCALE from rotation vectors
            assert d_y_descaled.shape == (6,)

            # Calculate scaling factor for dt
            # delta_t is ~10 times smaller than dt
            scale_dt = dt / delta_t
            myprint(f'scale_dt={scale_dt}')

            # Scale d_y so that we can combine with YOLO and use the actual dt
            d_y_descaled /= scale_dt
            myprint(f'd_y_descaled={d_y_descaled}')

            task2_1 = Pv @ d_y_descaled
            task2_2 = Ps @ d_y_descaled
            task2 = alpha * task2_1 + (1-alpha) * task2_2
            myprint(f'task2={task2}')

            # Manual integration of NODE
            y_next = y_start + delta_t*d_y
            
            # Update the start state for the next step of the NODE
            y_start = y_next

            # Get the next quaternion (NODE)
            rot_xyz_NODE = y_next[7:].clone().detach().cpu().numpy()
            quat_wxyz_NODE = from_tangent_plane(q_goal=goal_quat_wxyz, 
                                                r=rot_xyz_NODE, 
                                                scale=ROTVEC_SCALE)

            myprint(f'rot_xyz_NODE={rot_xyz_NODE}')
            myprint(f'quat_wxyz_NODE={quat_wxyz_NODE}')
        
            # Sum vBase_YOLO and task2
            combined_d_y = vBase_YOLO + task2

            next_pos = curr_pos + dt*combined_d_y[0:3]

            myprint(f'curr_pos={curr_pos}')
            myprint(f'next_pos={next_pos}')

            # Integrate EE angular velocity using exp map
            # to get next quaternion (using YOLO)
            # Here, its a one step integration with the exp map
            del_quat_wxyz = from_tangent_plane(
                q_goal=next_quat_wxyz, 
                r=(dt/2.0)*vBase_YOLO[3:6])
            ...
            self.move_robot(
                pos_xyz=next_pos, 
                quat_wxyz=quat_wxyz_NODE, ## TODO For now only calculate next ori from NODE
                rate=rate, 
                init=False)
